...
class Servicio(ModeloAuditable):
    ...
    def clean(self):
        if self.precio <= 0:
            raise ValidationError({'precio': gettext_lazy('El precio debe ser mayor que cero.')})
        if self.duracion_minutos <= 0:
            raise ValidationError({'duracion_minutos': gettext_lazy('La duracion debe ser mayor que cero.')})
