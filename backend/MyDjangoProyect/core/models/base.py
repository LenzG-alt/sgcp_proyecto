...
def validar_telefono(valor):
    ...
    if any(caracter not in caracteres_permitidos for caracter in valor):
        raise ValidationError(gettext_lazy('El telefono solo puede contener digitos y simbolos basicos.'))

    digitos = [caracter for caracter in valor if caracter.isdigit()]
    if len(digitos) < 7:
        raise ValidationError(gettext_lazy('El telefono debe contener al menos 7 digitos.'))
